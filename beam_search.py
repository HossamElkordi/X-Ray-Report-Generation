...
class BeamSearch(object):
    ...
    def _expand_visual(self, visual, cur_beam_size: int, selected_beam):
    ...
    def apply(self, visual, out_size=1, return_probs=False):
        if isinstance(visual, torch.Tensor):
            self.b_s = visual.size()[0]
        else:
            self.b_s = visual[0].size()[0]
        self.device = 'cuda'
        ...
        for t in range(self.max_len):
            visual, outputs = self.iter(t, visual, outputs, return_probs)
    ...
    def select(self, t, candidate_logprob):
    ...
    def iter(self, t: int, visual, outputs, return_probs):
        cur_beam_size = 1 if t == 0 else self.beam_size

        word_logprob = self.model.step(t, self.selected_words, visual)
        ...
        selected_idx, selected_logprob = self.select(t, candidate_logprob)
